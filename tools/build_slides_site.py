...
CHAPTER_TITLE = {"ps1_02": "Chapter 2 · Probability"}
...
def write_index(chapter, rows):
    ...
    body = []
    k = 0
    for sec, scenes in SECTIONS.get(chapter, [(CHAPTER_TITLE.get(chapter, chapter), [r["scene"] for r in rows])]):
        ...
        for s in scenes:
            ...
            body.append(f'<tr><td class="n">{k}</td><td><a href="{s}.html">{r["title"]}</a></td>'
                        f'<td class="s">{r["slides"]} steps</td></tr>')
    html = f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>{CHAPTER_TITLE.get(chapter, chapter)} · Probability and Statistics</title><style>{STYLE}</style></head><body><main>
<h1>{CHAPTER_TITLE.get(chapter, chapter)}</h1><p class="sub">Probability and Statistics · Walpole 9e · Ajou University</p>
<p class="how">Each page is one animation split into steps. Press → or Space (or click the arrow) to play the next step; ← goes back.
Fullscreen: F.</p>
<table><tr><th>#</th><th>Animation</th><th></th></tr>{''.join(body)}</table>
<p class="sub" style="margin-top:28px"><a href="../">All chapters</a></p></main></body></html>"""
...
def write_root_index():
    ...
    items = "".join(f'<tr><td><a href="{c}/">{CHAPTER_TITLE.get(c, c)}</a></td></tr>' for c in chapters)
    html = f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Probability and Statistics · Animations</title><style>{STYLE}</style></head><body><main>
<h1>Probability and Statistics · Animations</h1><p class="sub">Click-through versions of the lecture animations. Ajou University, 2026 Fall.</p>
<table>{items}</table></main></body></html>"""
