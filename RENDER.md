...
한 영상을 동작 단위로 끊어 → / 스페이스로 넘기는 RevealJS 페이지. manim-slides 가 manimgl 로 렌더한다.
    ...
    python tools/build_slides_site.py --chapter ps1_02 --module ... --only AdditionRule      # 한 씬만
    python tools/build_slides_site.py --chapter ps1_02 --module ... --skip-render            # 변환·목차만

- 씬 목록과 순서는 `_2026/probstat/slides_ps1_02.py` 의 `PAGES`. 기존 씬을 상속하고 `wait()` 가 0.5초
  이상이면 단계 경계로 삼는다(씬별 조정은 `STEP_AT`).
